const baseQuests = [{
  ...
  function animate() {
    ...
    if (keys['w'] && player.y - playerSpeed > 0) {
      player.y -= playerSpeed;
    }
    if (keys['s'] && player.y + playerSpeed < canvas.height) {
      player.y += playerSpeed;
    }
    if (keys['a'] && player.x - playerSpeed > 0) {
      player.x -= playerSpeed;
    }
    if (keys['d'] && player.x + playerSpeed < canvas.width) {
      player.x += playerSpeed;
    }
    if (keys[' ']) {
